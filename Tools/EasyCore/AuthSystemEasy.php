...
Class AuthSystemEasy {
    ...
    public function init(){
        ...
        $this->sessions = $GLOBALS['Commons']['sessions'];
    ...
    public function in_groups($groups_array){ // UnSecure !!! internal usage ONLY by STRING in Code !!!
    ...
    public function in_groups_by_id($groups_array){
        ...
        if (!$this->user) {
            ...
            $query = "SELECT 
                    ...
                    FROM auth_users_groups 
                    LEFT JOIN auth_groups ON auth_groups.id = auth_users_groups.group_id
                    WHERE auth_groups.id {$in} AND auth_users_groups.user_id = {$this->user['id']}
            ";
            $data = $this->db->sql_get_data($query);
