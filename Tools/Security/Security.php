...
Class Security
{
    ...
    static function string_easy_cleaner($input) {
        return preg_replace("/[^A-Za-zА-Яа-я0-9.,!?[:space:]]-_/","",$input);
    }
    ...
    static function input_cleaner_easy($input) {
        if (is_array($input)) {
            $return = Array();
            foreach ($input as $value) {
    ...
    function check_csfr_token_ajax_api() {

        if ($GLOBALS['Commons']['user']) {

            $user_id = $GLOBALS['Commons']['user']['id'];

            $headers = getallheaders();

            if (isset($headers['x-request-type']) AND $headers['x-request-type'] == 'AJAX_API') {

                // print_r($headers);

                if (!isset($headers['x-csrf-token'])) $headers['c-csrf-token'] = '';
                $csrf_token = $headers['x-csrf-token'];

                if ($this->aes256_decode($csrf_token, $this->csrf_token_key) != $this->csfr_token($user_id)) {
                    // echo $csrf_token.' ?? '.$this->aes256_decode($csrf_token, $this->csrf_token_key)." !== ".$this->csfr_token($user_id);
                    exit('Security Error #ct1');
                }

            } else {
                exit('Security Error #ct2');
            }
        };
