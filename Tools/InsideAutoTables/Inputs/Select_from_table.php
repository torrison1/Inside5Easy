...
class Select_from_Table {
    ...
    static function input_form($input_array)
	{
		...
		$variants[$i]['id'] = $row [$tmp_index];
  		...
  		if (!isset($input_array['select_attr'])) $input_array['select_attr'] = '';

  		$data .= "
  		<select name=\"".$input_array['name']."\" id=\"".$input_array['name']."\" class=\"input form-control selectpicker\" data-live-search=\"true\"".$input_array['select_attr'].">
