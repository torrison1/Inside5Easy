...
class Parent_select_custom {
    ...
    public function input_form($input_array)
    {
        ...
        return $this->c7_input_get_tree_from_res_select ($menu_res, $input_array['name'], $input_array['value'], $input_array['select_field']);	}

    // Parent Select Generator
    function c7_input_get_tree_from_res_select ($res, $select_name, $select_id, $field)
    {
        $data = '';
        $data .= '<select id="'.$select_name.'" class="'.$select_name.' input form-control selectpicker" name="'.$select_name.'" style="width: 350px;" data-live-search="true">';
        $data .= "<option value=\"0\">[0] MAIN</option>";
        ...
        while ($i <= $tree_i)
        {
            ...
            if (@$catalog_tree[$i]['parent_id'] == @$parent[$parent_step]) #if id has parent_id in current parent level (start in 0)
            {
                ...
                if ((@$now_output_signal == false)&&(@$catalog_tree[$i]['id'] > 0)) #if id has not printed
                {
                    ...
                    $data .= "<option value=\"".$catalog_tree[$i]['id']."\"".$select.">[".$catalog_tree[$i]['id']."] ".$data_prefix.$catalog_tree[$i][$field]."</option>";
                    $select="";
                    $data_prefix .= "->";
